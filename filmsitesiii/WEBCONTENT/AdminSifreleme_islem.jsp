...
String eski=request.getParameter("eadminsf");
String yenisifre=request.getParameter("yadminsf");
String yenisifretekrar =request.getParameter("yadminsft");
...
if(rs.next()){
	...
	String sifre=rs.getString("adminsf");
	if(hexString.toString().equals(sifre)){
		...
		if(yenisifre.equals(yenisifretekrar)){
			...
			boolean islemSonucu=islemler.updateDataAdmin(sayiNesnesi,id);
			
			if (islemSonucu==true){
				request.setAttribute("alert", "Şifre Güncellendi");
				RequestDispatcher rd1=request.getRequestDispatcher("denemeana.jsp");  
			...
			%>	<%String message = (String)request.getAttribute("alert"); %> 	
			 ...
			 Cookie yeniSifre = new Cookie("yeniSifre", request.getParameter("yadminsf"));
		...
		else
		{
			 request.setAttribute("alert", "Yeni Şifreler Aynı Değil");
				RequestDispatcher rd1=request.getRequestDispatcher("AdminSifreleme.jsp");  
				rd1.include(request, response);
